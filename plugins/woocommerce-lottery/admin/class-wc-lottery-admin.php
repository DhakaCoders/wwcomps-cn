	...
	public function product_write_panel() {
		...
		woocommerce_wp_text_input(
			array(
				...
				'desc_tip'          => 'true',
				'description'       => __( 'Max tickets sold per user', 'wc_lottery' ),
			)
		);
					...
					if ( $product_data ) {
						...
						if ( $product_data_type == 'lottery' ) {
							...
							for ( $i = 0; $i < $item_meta['_qty'][0]; $i++ ) {
								...
								delete_post_meta( $product_id, '_lottery_participant_id', $order->get_user_id() );
		...
		if ( $order ) {
			...
			if ( $order_items = $order->get_items() ) {
				...
				foreach ( $order_items as $item_id => $item ) {
					...
					do_action( 'wc_lottery_cancel_participation', $product_id, $order->get_user_id(), $order_id, $log_ids = null , $item, $item_id );
				}
			}
		}
	}
